...
import { MEAL_TIMES, LANE_HEIGHT, HEADER_HEIGHT, MEMO_ROW_HEIGHT, LABEL_WIDTH, fs } from '../constants';
...
export default function GanttChart({
  ...
  colWidth, fontScale = 1,
}) {
  // 文字サイズに合わせて行の高さも伸縮させる
  const laneHeight = Math.round(LANE_HEIGHT * fontScale);
  const headerHeight = Math.round(HEADER_HEIGHT * fontScale);
  const memoRowHeight = Math.round(MEMO_ROW_HEIGHT * fontScale);
  ...
  return (
    <div style={{ marginBottom: 2 }}>
      <div onClick={() => setCollapsed(!collapsed)} style={{
        ...
        <span style={{ fontWeight: 700, fontSize: fs(14) }}>📅 献立スケジュール</span>
        <span style={{
        ...
        {hasSelection && (
          <span style={{
            background: highlightColor || '#E53E3E', color: '#fff', fontSize: fs(10), padding: '1px 8px',
        ...
        >
          <div style={{ display: 'flex', minWidth: totalWidth + LABEL_WIDTH }}>
            ...
            <div style={{ width: LABEL_WIDTH, flexShrink: 0, position: 'sticky', left: 0, zIndex: 3, background: '#fff' }}>
              ...
              <div style={{ height: memoRowHeight }} />
              {MEAL_TIMES.map((mt) => (
                <div key={mt.key} style={{
                  height: laneHeight * laneRowData[mt.key].maxRows, display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: fs(11), color: mt.color + '88', fontWeight: 600, flexDirection: 'column', lineHeight: 1,
                }}>
                  <span style={{ fontSize: fs(14) }}>{mt.icon}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ position: 'relative', flex: 1 }}>
                ...
                {dates.map((d) => {
                  ...
                  return (
                    <div key={d} style={{
                      ...
                      fontSize: fs(11), fontWeight: isToday ? 700 : 500, lineHeight: 1.3,
                    }}>
                      <div style={{ fontSize: fs(9) }}>{['日','月','火','水','木','金','土'][dow]}</div>
              ...
              {/* Memo row */}
              <div style={{ display: 'flex', height: memoRowHeight, borderBottom: '1px solid #eee' }}>
                {dates.map((d) => (
              ...
              {MEAL_TIMES.map((mt, laneI) => {
                ...
                return (
                <div key={mt.key} style={{
                  height: laneHeight * laneMaxRows, position: 'relative',
                    ...
                    .map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          style={{
                            position: 'absolute', top: rowIdx * laneHeight + 3, left: left + 2,
                            width: Math.max(width, 30), height: laneHeight - 6,
                            ...
                            fontSize: fs(11), fontWeight: 600, color: isResizing ? gc + '66' : gc,
                        ...
                        >
                          ...
                          <span style={{
                            ...
                            textOverflow: 'ellipsis', padding: '0 3px', fontSize: fs(10),
                          }}>
                          ...
                          <div
                            ...
                            style={{
                              ...
                              fontSize: fs(10), color: gc, fontWeight: 700, touchAction: 'none',
                            }}
                          >
                            ⋮
                          </div>
                  ...
                  {resizeGhost && resizeGhost.laneIdx === laneI && (
                    <div
                      style={{
                        position: 'absolute', top: resizeGhost.rowIdx * laneHeight + 3, left: resizeGhost.left + 2,
                        width: Math.max(resizeGhost.width, 30), height: laneHeight - 6,
                        ...
                        fontSize: fs(9), color: getGenreColor(resizeGhost.recipeId), fontWeight: 700,
                      }}
                    >
